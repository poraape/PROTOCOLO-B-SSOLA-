...
export const SCENARIOS_DATA: Scenario[] = [
  {
    ...
    treeTraversal: [
      { step: 1, nodeId: 'P1_P3', actor: 'professor', action: 'Conversa discreta após a aula.', rationale: 'Mudança de padrão exige investigação.', alertTriggered: 'A02' },
      { step: 2, nodeId: 'TRIAGEM_RAPIDA', actor: 'professor', action: 'Classifica como risco moderado, sem emergência.', rationale: 'Sem risco imediato, foco em causa externa.' },
      { step: 3, nodeId: 'ESCUTA_INTERNA', actor: 'professor', action: 'Aluno relata trabalho noturno.', rationale: 'Escuta qualificada permitiu identificar causa externa.' },
      { step: 4, nodeId: 'R48', actor: 'coordenação', action: 'Aciona fluxo de trabalho infantil e família.', rationale: 'Violação de direitos exige escalonamento.' },
      { step: 5, nodeId: 'CRAS_REFERRAL', actor: 'coordenação', action: 'Encaminha CRAS para proteção social básica.', rationale: 'Vulnerabilidade econômica sem violência grave.' },
      { step: 6, nodeId: 'INTERNAL_ADAPT', actor: 'professor + coordenação', action: 'Ajusta prazos pedagógicos.', rationale: 'Preserva vínculo escolar.' },
      { step: 7, nodeId: 'REGISTRO', actor: 'coordenação', action: 'Registra ações e encaminhamentos.', rationale: 'Proteção legal e histórico.', alertTriggered: 'A11' }
    ],
  ...
  },
  {
    ...
    treeTraversal: [
      { step: 1, nodeId: 'S3', actor: 'professora', action: 'Convida para sala reservada.', rationale: 'Sem exposição pública.', alertTriggered: 'A02' },
      { step: 2, nodeId: 'ESCUTA_PROTEGIDA', actor: 'professora', action: 'Faz pergunta direta sobre risco suicida.', rationale: 'Pergunta direta abre canal de cuidado.', alertTriggered: 'A16' },
      { step: 3, nodeId: 'R10_TRIAGEM', actor: 'coordenação', action: 'Classifica como risco alto sem iminência.', rationale: 'Define encaminhamento prioritário.' },
      { step: 4, nodeId: 'R11_R07', actor: 'coordenação', action: 'Não deixa sozinha e remove objetos de risco.', rationale: 'Segurança imediata.', alertTriggered: 'A10' },
      { step: 5, nodeId: 'CAPS_IJ_REFERRAL', actor: 'direção', action: 'Encaminha ao CAPS IJ com carta.', rationale: 'Serviço especializado para adolescente.' },
      { step: 6, nodeId: 'REGISTRO_RESTRITO', actor: 'coordenação', action: 'Registra em ficha restrita.', rationale: 'LGPD e continuidade de cuidado.', alertTriggered: 'A12' },
      { step: 7, nodeId: 'FOLLOWUP_PLAN', actor: 'coordenação', action: 'Define monitoramento semanal.', rationale: 'Evita encaminhamento sem retorno.', alertTriggered: 'A03' }
    ],
  ...
  },
  {
    ...
    treeTraversal: [
      { step: 1, nodeId: 'F7', actor: 'merendeira', action: 'Reconhece sinal e aciona apoio.', rationale: 'Primeira resposta rápida salva tempo.' },
      { step: 2, nodeId: 'C3_GLUCOSE', actor: 'agente de organização', action: 'Oferece açúcar apenas se consciente.', rationale: 'Segurança clínica.', alertTriggered: 'A18' },
      { step: 3, nodeId: 'FAMILY_CONTACT', actor: 'secretaria', action: 'Comunica família imediatamente.', rationale: 'Obrigatório em evento de saúde.' },
      { step: 4, nodeId: 'ESCALATE_SAMU', actor: 'direção', action: 'Aciona SAMU se sem melhora em 10 min.', rationale: 'Sem resposta = risco iminente.' },
      { step: 5, nodeId: 'REGISTRO', actor: 'secretaria', action: 'Documenta cronologia completa.', rationale: 'Continuidade do cuidado.' }
    ],
  ...
  },
  {
    ...
    treeTraversal: [
      { step: 1, nodeId: 'D1_D7', actor: 'direção', action: 'Preserva evidências e aciona coordenação.', rationale: 'Prints são evidência de possível crime.' },
      { step: 2, nodeId: 'VITIMA_FIRST', actor: 'coordenação', action: 'Acolhe vítima em privado.', rationale: 'Vítima primeiro.', alertTriggered: 'A06' },
  ...
  },
  {
    ...
    treeTraversal: [
      { step: 1, nodeId: 'D9_LOCKDOWN', actor: 'porteiro', action: 'Fecha portão e alerta direção.', rationale: 'Primeira resposta de contenção.' },
      { step: 2, nodeId: 'PM_190', actor: 'direção', action: 'Aciona PM 190 imediatamente.', rationale: 'Segurança pública em curso.' },
      { step: 3, nodeId: 'LOCKDOWN_INTERNO', actor: 'direção', action: 'Comanda lockdown interno.', rationale: 'Reduz exposição ao risco.' },
      { step: 4, nodeId: 'COMMUNICATION_CONTROL', actor: 'direção', action: 'Centraliza comunicação e evita ruído.', rationale: 'Prevenção de pânico.' },
      { step: 5, nodeId: 'POST_CESSATION', actor: 'direção', action: 'Comunica famílias após cessação.', rationale: 'Transparência segura.' },
      { step: 6, nodeId: 'TRAUMA_FOLLOWUP', actor: 'coordenação + professores', action: 'Monitora sinais de trauma nas semanas seguintes.', rationale: 'Violência armada pode gerar TEPT.' },
      { step: 7, nodeId: 'REGISTRO_DE_LESTE', actor: 'direção', action: 'Registra e comunica DE Leste 1.', rationale: 'Governança institucional.' }
    ],
  ...
  },
  {
    ...
    treeTraversal: [
      { step: 1, nodeId: 'V7_S5', actor: 'funcionária', action: 'Interrompe discriminação e chama AEE.', rationale: 'Proteção imediata e suporte especializado.' },
      { step: 2, nodeId: 'CRISE_REGULACAO', actor: 'professor AEE', action: 'Aplica estratégia de regulação conhecida.', rationale: 'Evita escalada da crise.' },
      { step: 3, nodeId: 'REGISTRO_DISCRIMINACAO', actor: 'coordenação', action: 'Registra formalmente o episódio.', rationale: 'Capacitismo é violência.', alertTriggered: 'A11' },
  ...
  },
  {
    ...
    treeTraversal: [
      { step: 1, nodeId: 'S6_P2', actor: 'professor', action: 'Abordagem reservada e acionamento da coordenação.', rationale: 'Evita exposição e ruptura de vínculo.', alertTriggered: 'A02' },
      { step: 2, nodeId: 'ESCUTA_COORDENACAO', actor: 'coordenação', action: 'Escuta não punitiva com aluno.', rationale: 'Favorece adesão.', alertTriggered: 'A16' },
      { step: 3, nodeId: 'TRIAGEM_SUBSTANCIAS', actor: 'coordenação', action: 'Distingue uso ocasional vs problemático.', rationale: 'Define fluxo correto.' },
      { step: 4, nodeId: 'CAPS_AD_REFERRAL', actor: 'coordenação', action: 'Encaminha CAPS AD com carta escolar.', rationale: 'Dependência é caso de saúde.' },
      { step: 5, nodeId: 'FAMILIA_CONTACT', actor: 'coordenação', action: 'Engaja família com consentimento do adolescente.', rationale: 'Aliança terapêutica.' },
      { step: 6, nodeId: 'CRAS_SOCIAL', actor: 'coordenação', action: 'Aciona CRAS se vulnerabilidade econômica.', rationale: 'Determinantes sociais importam.' },
      { step: 7, nodeId: 'PEDAGOGICAL_PLAN', actor: 'coordenação + professor', action: 'Plano pedagógico de manutenção do vínculo.', rationale: 'Reduz risco de evasão.' }
    ],
