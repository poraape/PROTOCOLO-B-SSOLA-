...
export const NetworkPage: React.FC = () => {
  ...
  const mappableServices = useMemo(
    () => orderedServices.filter((service) => hasCoordinates(service) && service.geoStatus === 'VERIFICADO'),
    [orderedServices]
  );
  const listOnlyMode = shouldUseListFallback(orderedServices.length, mappableServices.length);
        ...
        <section className="card-elevated mb-5">
          ...
          {showMap && (
            <div className="mt-3 h-[320px] overflow-hidden rounded-xl border border-border">
              {mappableServices.length ? <NetworkMap services={mappableServices} /> : <div className="flex h-full items-center justify-center text-sm text-muted">Sem coordenadas verificadas para os filtros atuais.</div>}
            </div>
          )}
        </section>
      )}
