...
export const QuestionStep: React.FC<QuestionStepProps> = ({ node, onSelect }) => {
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <h2 className="text-3xl font-extrabold leading-tight text-slate-900">{node.question}</h2>
      <p className="mt-2 text-sm text-slate-600">Escolha uma opção para continuar o protocolo.</p>

      <IndicatorsAccordion items={node.indicators || node.severityCriteria} />
      ...
      <div className="mt-6 grid gap-3">
        {node.options.map((option) => (
          <button
            ...
            className="rounded-2xl border border-slate-300 bg-white px-5 py-4 text-left text-base font-semibold text-slate-900 transition hover:border-[#007AFF] hover:bg-blue-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500"
          >
            {option.label}
          </button>
        ))}
